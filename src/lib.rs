...
//!
//! nixvault is currently a binary-only crate, but the `cargo test --doc`
//! pre-push hook hard-errors with "no library targets found in package
//! `nixvault`" when no library target exists — doctests can only live on a
...
///
/// ```
/// assert_eq!(nixvault::stub_until_first_real_doctest(), ());
/// ```
