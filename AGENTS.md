...
`CLAUDE.md` is the authored source of truth for this project's rules — what nixvault is,
...
Non-negotiable: this project is self-contained. The docs describe **only** nixvault, and every fact
must stand alone in this repository. An external reference is allowed only where it names *upstream*
technology that is part of nixvault's own stack (Nix, microvm.nix, NixOS modules, direnv) — never as
a load-bearing dependency on another *project*, tool, external shelf, or personal path. If external
knowledge is required to understand, build, or operate nixvault, copy its essential substance into
