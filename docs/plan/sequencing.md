...
- Many workspaces per sandbox — `ADR-0108` for the declaration split and the scope N16 gains when more than one tree is mirrored, `ADR-0109` for what a command does outside a declared tree, with `ADR-0100` read a second time as the mirroring both generalize and `ADR-0009` fixing why the path is injected at launch. Consumed by [slice 020](./slices/020-many-workspaces-in-one-sandbox/README.md), which is where several projects first share one VM.
- Sandbox identity and binding — `ADR-0107` as the record that replaces `ADR-0011`, `ADR-0029`, `ADR-0043`, and `ADR-0054`, with `ADR-0040`, `ADR-0052`, and `ADR-0053` bounding it and `ADR-0108` supplying the single-owner rule its derived index needs. Consumed by [slice 021](./slices/021-the-manifest-is-the-sandbox/README.md), which enacts a decision already recorded rather than making one.

  This pair leads Phase 2 and cannot slide behind slices 005, 006, 008, 018, 024, or the four fleet and lifecycle slices below. Those touch CLI plumbing, status enforcement, image ownership, installation, and every surface that reads per-sandbox state or renders a row per sandbox, and every one of them written before the rekey is written against `<project-id>` and the identity machinery slice 021 deletes — the same argument that put slice 015 ahead of slice 005, and worth stating here so it is not rediscovered.
- Capacity reporting and admission — `ADR-0036`, whose per-VM scope half slice 002 already realized and whose checking half is still unbuilt, with `ADR-0082` fixing what a memory figure may be measured on and `ADR-0023` bounding the probe catalog the check either joins or gates beside. Consumed by [slice 025](./slices/025-the-fleet-is-visible/README.md) for the reading and [slice 026](./slices/026-a-start-checks-the-room/README.md) for the decision taken on it; `ADR-0107` and `ADR-0108` are why both arrive after the rekey rather than before it, since a row keys on the manifest and carries a set of workspaces.
