...
- Declared mounts — `ADR-0020`, `ADR-0021`, `ADR-0071`, with `ADR-0027` bounding the confinement each new share's daemon inherits. Consumed by [slice 019](./slices/019-declared-mounts-reach-the-guest/README.md), which was sequenced ahead of slice 005 because the declaration surface was specified, typed, emitted, merged, and consumed by nothing; the slice is `done` and the share list the two slices after it grow now exists. The file-mount confinement residual it accepted (`Q-031`) is consumed by [slice 022](./slices/022-a-file-mount-serves-only-its-file/README.md), sequenced ahead of slice 020: the residual widens with every mount a shared sandbox adds, so it closes before the share list grows.
