...
- Self-supply and lock scope — `ADR-0058`, `ADR-0059`, plus the ADR the slice itself records to exit [`Q-014`](./open-questions.md) and [`Q-027`](./open-questions.md). Consumed by [slice 015](slices/015-the-binary-supplies-itself/README.md), sequenced ahead of slice 005: `Q-027` was met in ordinary development the day slice 014 closed, the flawed shape it exposes — a generated flake pinning a Nix-built `viv` against the installed one — sits under every later slice's verification, and removing a self-dependency gets more expensive with each slice built on top of it.
