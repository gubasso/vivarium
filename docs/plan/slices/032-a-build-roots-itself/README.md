...
Implemented 2026-08-25 in one pass under the appetite, remainder included: the core items 1 through 8 and the whole ordered remainder — `viv gc`'s sweep, `generations activate` and `rollback`, and `viv start --generation <n>` — with nothing cut. The host evidence is in the harness [findings register](../../../reference/microvm-verification-harness.md): `tests/host/generations-check` first complete run `PASS=16 FAIL=0 SKIP=0 RECORD=6`, four real collections at 13–24 seconds each reclaiming an unrooted canary beside a held build, and `workflow_24_generations_retention` green through the acceptance harness.

Three moves went past the items as written, each recorded where it belongs. Item 3's reconciliation took the first branch and one step more: `last-build` died and its reader resolves the profile's `current`, and `running-build` moved from the data root to the state root, because the data root is spec/02's home for pinned inputs and a boot-provenance record is what a run produces — spec/02 now states the two-record split. Second, an unchanged build appends no generation: a re-evaluation that reproduces the current output is the same build under N4's freshness key, and spec/11 now says so rather than leaving the rule to the code. Third, `prune` never selects the generation `current` points at, whatever the retention argument says — spec/11 owns that rule; the alternative silently breaks `--no-rebuild`, which is this slice's defect class wearing a new verb.

One near-miss worth its line: `viv gc`'s first draft read the collector's last stderr line as its accounting summary, and on the target host that line is a hard-linking housekeeping note — the accounting goes to stdout, measured with a canary deletion. The verb now relays stdout, and the register's method note already owned the lesson — assert on the thing the check is named after.
