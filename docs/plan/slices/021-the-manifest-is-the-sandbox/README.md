...
Reviewed a second time on 2026-08-22, and the second pass paid for itself twice. Two rounds returned six findings, two of which the first review had not reached and two of which were regressions in the first review's own repairs.

The one that matters most is a strengthening of item 3's decision. The index stamped each manifest with its modification time alone, which is what shaping chose and what the first review accepted. A rewrite that preserves the mtime — a `cp -p`, an archive restore, or two edits inside one tick on a filesystem with coarse granularity — left every stamp equal, so the cached index was served and a manifest that had since claimed the invoking directory never became a candidate. Resolution then reached the previously indexed owner and said nothing, where the honest answer was that two manifests now claim the directory. Reaching the wrong sandbox silently is the worst failure this cache can produce, so the stamp now carries a hash of the manifest's own bytes beside the mtime. It uses the standard library's own hasher rather than a new dependency: the hazard is accidental change rather than forgery, and a hash that disagrees with a previous run's costs a rebuild, which a rebuildable cache can afford and a wrong hit is what it cannot.

The second is an ownership rule the slice needed and nobody had written down. `vivarium.workspaces` was declared like `vivarium.mounts`, as a list whose layers concatenate, so an image, a piece, or an `extends` module could contribute a workspace. That contradicts the resolution model rather than merely surprising it: ownership is decided from manifest text alone, because the derived index scans each manifest's `[[workspaces]]` rows and the undeclared-directory refusal is a `78` for exactly that reason. A workspace a shared layer contributes would own a tree nothing could resolve to it, and the boot record would carry a tree the next invocation's reuse check does not know about — which is how it was found, as a live regression in the repaired reuse predicate. Only a manifest may declare a workspace now, refused at `65` naming the layer, and the refusal counts an empty declaration too: the report carries only a key a layer actually wrote, so `mkForce []` from a shared layer is not an abstention but the one shape that silently removes the owner's own rows.

Two smaller repairs came from the same rounds. `viv doctor` erased the second-claimant finding through an `.ok()`, so the single condition ADR-0109 exists to explain rendered as no manifest being bound — and after that was fixed, its stderr note still said no manifest declared the directory while the findings above it named two. Both halves are now asserted rather than assumed. And `status` and `stop` resolved the runtime root before the manifest, so an undeclared directory on a host with an unusable `${XDG_RUNTIME_DIR}` answered `77` about the host rather than the shared `78` about what the user did.

Two lessons are worth keeping past this slice. A rule that reads an evaluated report can pass every unit test while never firing, so the ownership refusal is exercised end to end through a real evaluation as well; that is the assumption-shaped check `AGENTS.md` warns about, met in the wild. And a trial that asserts only stdout and an exit code does not cover a contradictory note on stderr, which is where the doctor regression hid.

Gates after both rounds, all through [`tests/host/heavy-run`](../../../../tests/host/heavy-run): the `pre-push` profile green twice at 581 and 544 seconds, `nix flake check` over all four contract lanes, and the complete pre-commit hook set.
