...
- Landed within the reshaped appetite. Two things were discovered rather than shaped, and both were built because the slice's own `Acceptance` could not hold without them. The launch specification carried no guest mount point, no per-target lock path, and no guest process environment, so nothing on the host could name the workspace cwd a session starts in, the supervisor's teardown sweep would have aborted on the lock spec/12 places in the runtime directory, and `viv exec -- true` could not have resolved `true` at all — the guest agent clears the environment before every spawn and the guest module set no `PATH`. All three are now derived from the guest module and carried through a schema-3 launch specification, and `tests/nix/contract.sh` compares each against the guest's own `/etc/fstab` and `/etc/set-environment` rather than against a second copy of a constant. That check earned its place immediately: it caught a rendered `PATH` that expanded `${XDG_STATE_HOME}` to its conventional value where the guest's own script drops the segment.

- One acceptance clause holds without a host trial, and is named here rather than left implied. "While the control socket is not yet ready, `viv exec` SHALL fail with a stated reason rather than hang" is enforced by three bounded budgets — the reuse ping, the wait for a live VM's agent, and the session's own connect — each of which produces a named diagnostic and a code on expiry. What has no trial is a host that reaches one of them, because provoking it means a VM whose process is alive and whose agent will not answer, and nothing in the product creates that state on request. The other five clauses are demonstrated on a capable host, `74` included.

- Ordered remainder, cut in full and not built. The live-session count `viv status` reports, the host-subdirectory cwd mapping, and explicit `Signal` frames on the non-PTY path are all absent rather than partial. The first two are recorded against their rows in [`../../../reference/implementation-status.md`](../../../reference/implementation-status.md).

- One clause of spec/12 step 3 has no host-side expectation to compare against, and is not implemented. The step asks that `boot.json` match "the project identity, running generation/store path, backend, and workspace host path"; `boot.json` carries no generation or store path, and neither `viv exec` nor `viv shell` selects a generation, so there is nothing for that half of the comparison to be wrong about. The four decidable fields are matched, and a record naming another project refuses rather than tearing down a VM this project does not own.
