...
4. Land the adversarial trial: from inside the guest, as root, remount the file mount's tag and attempt to read and to write a sibling of the declared file; both attempts fail. The slice 019 trials (`workflow_17_declared_mounts_round_trip` among them) keep passing unchanged, or their `Revisions`-recorded change is part of item 3's enactment.
5. Rewrite the accepted-residual paragraph in [`../../../reference/spec/06-workspace-and-project-environment.md`](../../../reference/spec/06-workspace-and-project-environment.md) into the guarantee (or the narrowed rule) the decision gives, and give [`Q-031`](../../open-questions.md) its exit.
...
When a regular-file mount the decision admits is declared, a guest-root attempt to reach any other entry of the source's parent directory through that share SHALL fail, for reading and for writing, and a trial SHALL demonstrate both from inside a booted guest.
...
Every acceptance assertion above holds and is demonstrated by the trial it names, the decision ADR is `Implemented`, [`Q-031`](../../open-questions.md) carries the exit this slice gives it, the spec/06 residual paragraph is replaced, and the [`milestones.md`](../../milestones.md) row flips to `done`.
