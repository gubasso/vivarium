...
Items 3 and 4 landed in the second checkpoint. One resolution routine now owns flag, environment, and the then-authored third precedence rung; it parses the selected manifest, resolves every declared workspace, returns the matched tree and the temporary anchor separately, and produces the undeclared-directory finding consumed by every resolving verb and pedagogically by `viv doctor`. The temporary path is an interim sandbox anchor and never an implicit workspace; slice 021 subsequently removed that anchor and the old identity mechanism. `viv status` and `viv stop` now pay the same manifest-parse precondition as the other resolving verbs and may newly return `78` for broken selected text; that is ADR-0109's deliberate consistency cost rather than a compatibility path around the check. Reuse compares the complete tag-to-host-path map already held by the resolved manifest and `boot.json`, so a live VM still skips preflight, evaluation, and build even when its anchor lies outside the workspace set.

The capable-host premises were rechecked before this continued round. `/dev/kvm` was present, `$XDG_RUNTIME_DIR` was the absolute writable `/run/user/1000`, and the configured drive had about 1.7 TiB free. The user manager answered `degraded`, not `running`: its bus and transient-unit operations were usable, while six unrelated backup services were failed. This differs from the earlier observation and is recorded rather than promoted into a product premise. No pre-existing `vivarium-*` unit was loaded.

The phase-2 Rust gate passed all unit and integration tests. Its first workflow pass saw the known DNS exit-6 and PTY-size flakes once; each passed on two focused reruns. A separate identity fixture failure reproduced, was repaired by giving its two manifest rows disjoint explicit workspace owners, and passed twice. The final required-gate workflow run passed `28/28` with no skips in `567.960` seconds. The phase-3 focused acceptance run passed both the expanded refusal trial and `workflow_20_many_workspaces_one_sandbox` in `140.585` seconds.

The first timed pre-push attempt exposed a stale capable-host fixture: `guest_agent_host` still passed the pre-schema-10 runner arguments and then treated a rendered launch specification as a booted VM. It reproduced twice, so it was repaired in this slice rather than classified as a flake: the fixture supplies `--workspace ws0`, names the running supervisor, and performs ADR-0102's separate `viv start --spec` handoff. The repaired trial passed twice alone in `13.894` and `13.615` seconds. The two complete `kind(test)` runs then passed `56/56`, with no skips, in `564.96` and `558.22` seconds. Items 1 through 6 landed; neither ordered-remainder cut was needed.
