...
When `viv update` runs under the tool-owned lock, it SHALL move only the inputs named and what follows them, SHALL report each reported row's before and after, and SHALL NOT build.
...
When a user moves a guest component's pin and rebuilds, the next `start` SHALL boot a VM carrying the moved component, with no vivarium release required and no pin moved outside the requested inputs and the public inputs that follow them.
...
A pin move followed by a rebuild is the same shape the closed `Q-023` priced at a cold first boot — about twenty-nine minutes against a twenty-second reuse, preserved in the [findings register](../../../reference/microvm-verification-harness.md) when slice 032 took its exit — so item 7's cost is known before the lane runs rather than discovered inside it. That is the second reason item 7 names the disk preflight rather than leaving capacity to chance.
...
Recorded 2026-08-26, at start, before any code. The owner reshaped item 2 from a redirect knob to a composition seam. A directory-form image may carry its own real `flake.nix` beside its `default.nix`; `image = "<name>"` selects both the module layer and that base; the generated flake takes the base as a real flake input named after the image and follows `nixpkgs` and `microvm` out of it when the base declares them, keeping vivarium's own reference for whichever it does not. The user's flake contributes inputs only — vivarium keeps the outputs skeleton, and a later slice may open a named output attribute for modules. The alternatives offered and declined were an attrset file that is never a flake, and a real flake consumed as an inert tree. Consequences the owner accepted with the choice: the base's own `flake.lock`, when present, seeds the first resolution and is dead weight after — the tool-owned lock stays the one effective lock — which amends `ADR-0059` and `ADR-0074` without replacing either decision; and an in-place edit to the base is refused at `78` until `viv update` moves the pin, which is N3 holding. Item 2's acceptance phrase `no unnamed pin moved` is revised to `no pin moved outside the requested inputs and the public inputs that follow them`, because updating the base input moves the baselines that follow it, and reporting that movement is the honest form of the same guarantee. The appetite stays 4 sessions; nothing is pre-cut, and a cut, if the appetite binds, follows the ordered remainder from the bottom.

Implemented 2026-08-26 in one pass under the appetite, remainder included, from the owner's reshaped item 2. The pass opened with the Nix behavior experiments the reshape demanded, and their findings moved the design before any code: the base input is rendered `path:./images/<name>` — relative and hashless, so the base's text is a layer — because the absolute form was measured silently serving stale content against its lock, which also dissolved the planned stale-base classifier (there is no error to classify); a base's own `flake.lock` re-seeds every re-lock, so `viv update` reports held pins with a stderr note rather than moving past them; `nix flake update` on a `follows` alias warns and moves nothing, so a requested followed baseline is translated to the base input owning its node; and the follows-plus-override shape is silently ignored by Nix, so the renderer never emits it and the composed-lock check (`lock.baseline-split`) owns the coherence the dropped edge carried. The full record is the two 2026-08-26 entries in the harness [findings register](../../../reference/microvm-verification-harness.md).

Two acceptance sentences were revised in this pass, recorded here per the charter: "move only the inputs named" gained "and what follows them", and "no unnamed pin moved" became "no pin moved outside the requested inputs and the public inputs that follow them" — updating a base necessarily moves the baselines that follow it, and reporting that movement is the honest form of the same guarantee. A row's `changed` is decided over its whole resolved subtree, because the pin a user actually moves can sit below the hashless base node.

Item 5's premise failed: the `ADR-0049` feature floors were never written as assertions — nothing in `nix/` asserts one — so there was no diagnostic to improve, and the item closed as that finding with `Q-037` raised in [open-questions](../../open-questions.md). The acceptance's floor assertion is an `If` whose antecedent is empty today; that vacuity is named here rather than read as a pass.

Deviations worth carrying: `viv update` takes the same per-target lock `start` does, so spec/14's update row gained `75` and the lock diagnostic's wording was generalized; no trial exercises two lock holders racing — the enforcement is the shared `TargetLock`, and the gap is named in [implementation-status](../../../reference/implementation-status.md). Host evidence: `tests/host/update-check` first complete run `PASS=7 FAIL=0 SKIP=0 RECORD=3` — first update under a second, build-and-boot 49s warm, rebuild after the move 22s, the moved component read from inside the guest and the move repeated with a third revision — and `workflow_18_update_usage_surface` plus `workflow_18_update_moves_the_pin` green through the acceptance harness.
