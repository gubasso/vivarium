...
When no manifest is selected, project probes SHALL skip with reason `no-manifest-bound`, one stderr note SHALL say that no manifest declares the workspace, and all host probes passing SHALL still exit `0`.
