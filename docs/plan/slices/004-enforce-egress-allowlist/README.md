...
Recorded 2026-08-14, after close: the `Out of scope` line `Wildcard support` was overtaken by the slice's own acceptance rather than honored. The pattern grammar [`05-networking-and-egress.md`](../../../reference/spec/05-networking-and-egress.md) fixes includes the `*.`/`**.` wildcard forms, and session 1 matched that grammar in full, refused-not-narrowed, in [`src/net/allowlist.rs`](../../../../src/net/allowlist.rs) — so wildcards ship, matched at the gating resolver like any exact name, and the exclusion never described a cut that happened. The line stays as written because scope text freezes at start; this entry is the correction's home.
