...
The coding-agent run was attempted on 2026-08-13 and is blocked, for a reason no trial in this slice could have surfaced. A project was bound to a manifest carrying `pkgs.claude-code`, the guest built and booted, and everything this slice owns held under hand inspection: the workspace mirrored at the host's own absolute path, an edit crossed in both directions, a guest-created file read back on the host as the invoking user, and `git status` and `git log` resolved against the mounted tree. The agent is installed and runs — `claude --version` reports `2.1.224` inside the guest. It has nothing to dial. `ip -o link` reports `lo` alone, `ip route` reports nothing, and `curl` exits `6` for every host, because the connectivity [`../../../reference/spec/05-networking-and-egress.md`](../../../reference/spec/05-networking-and-egress.md) specifies is slice 004's to build rather than slice 004's to filter. So `Done when`'s coding-agent clause is not merely unperformed, it is unreachable from this slice, and the row stays `active` for a reason that now has a measurement behind it rather than an omission. [`../../sequencing.md`](../../sequencing.md) carries what this costs the acceptance table, and the two questions it raised are [`Q-021`](../../open-questions.md) and [`Q-022`](../../open-questions.md).

The same run re-measured the durability fix and reproduced it. An unsynced write into the guest home survived a `viv stop` and `viv start` twice, the second time including a nested `~/.config` subtree, which is the shape an agent's own credentials would take and the reason the volume matters at all here. `viv stop` completed in 1.61 and 1.50 seconds against the ten-second grace, matching what item 3 recorded, and a warm `viv start` took 26.31 and 24.57 seconds against a cold first boot of roughly 29 minutes. Two observations rather than one, because the claim is about reliability and one clean run is possibility.

One authoring constraint was found and belongs with the slice that met it, because anyone reaching for the same milestone meets it first. `claude-code` carries an unfree license and a guest system is an ordinary NixOS evaluation, so the first boot failed outright at evaluation rather than at runtime. Vivarium offers no knob for this and should not: the piece that asks for the package carries `nixpkgs.config.allowUnfreePredicate` scoped to that package, which is a layer deciding its own policy through the merge exactly as spec/04 intends. Named here rather than in a guide, because the shipped [`../../../../examples/pieces/`](../../../../examples/pieces/) library carries no unfree package and so cannot demonstrate it.
