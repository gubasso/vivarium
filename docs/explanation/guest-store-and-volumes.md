...
This page describes the accepted design; the volume half of it runs today and the store-collection half does not. See [implementation status](../reference/implementation-status.md) for which is which.
...
Which layer declared a volume is recorded rather than re-derived. A volume's declaring layer, and the guest path an orphan used to occupy, exist only inside a merged evaluation — and the two verbs that report them are read-only rows with a single failure mode, while an evaluation has four. So the evaluating command writes what it learned to a per-target record beside the images, and the reading commands read it. The record is stale by construction and is therefore believed conditionally: a volume the manifest itself declares is live on sight, and one only a piece declared is believed while the composition that produced it — the image, the pieces, and `extends` — is unchanged. Dropping a piece retires every row that record vouched for, so its volumes become reclaimable at once. The residual error is one-directional and worth naming: a piece that changes its own volume declarations while the manifest's composition is unchanged keeps an old volume alive until the next start refreshes the record. Late, never false, which is the bias a verb that deletes user data has to have.
