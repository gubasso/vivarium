...
- The workspace-owner index publishes without a lock, and that it is safe to do so is argued rather than measured. The argument is short and probably right: `rename(2)` within a directory is atomic, every writer derives an equivalent value from the same library, and the index is rebuildable, so a stale or missed read costs a rebuild rather than a wrong answer. What has never been observed is two `viv` invocations racing on one cache root — the unit tests exercise one writer at a time. Two things would settle it: a repeated concurrency trial asserting that every reader sees a whole index and that no temporary sibling survives, or a decision that rename semantics are load-bearing enough to rest on. Until one of them happens the claim is a reasoned expectation, and the crash-window residue [`../reference/spec/02-config-and-xdg-layout.md`](../reference/spec/02-config-and-xdg-layout.md) now names is the part of it nothing cleans up.
