...
The word "host" there is worth reading carefully, because the sentence is easy to misread as a claim about the user's own packages. The paths a guest shares are the ones **vivarium built**, and they are on the host for that reason alone — the sharing works identically on a machine whose owner installs software with `apt` and has never run Nix. What is _not_ shared is what a project's own inner environment pulls in, and the granularity there is the store path rather than the package name: a dependency the image declares is used in place, while the same dependency named by a project pinning a different `nixpkgs` is a different hash and is materialized separately. Both outcomes are Nix behaving correctly, and the second is the honest per-project cost of an inner environment that answers to nobody's pin but its own. It lands on the project's store volume ([`../decisions/ADR-0087-the-inner-store-persists-on-its-own-volume.md`](../decisions/ADR-0087-the-inner-store-persists-on-its-own-volume.md)), so it is paid once rather than on every boot.
