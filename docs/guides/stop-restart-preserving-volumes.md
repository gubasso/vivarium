...
Two trials cover this: `workflow_07_volume_list_requires_manifest` in [`local_workflows.rs`](../../tests/local_workflows.rs) checks the missing-manifest and conflicting-flag surface, and `workflow_07_stop_restart_preserving_volumes` in [`boot_workflows.rs`](../../tests/boot_workflows.rs) checks the default and named images, stop idempotence, the stopped state, and a warm restart.
