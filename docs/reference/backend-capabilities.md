...
`nix/flake.lock` is the local authority for the product input set. As checked on 2026-08-06 it pins:
...
The same v52.0 hybrid-vsock host endpoint is an ordinary Unix stream. A host-opened connection writes `CONNECT <guest-port>\n`; the VMM replies `OK
<ephemeral-host-port>\n` when the guest connection is established. The host consumes that bounded acknowledgement before exchanging application bytes, and the numeric acknowledgement port is not the requested guest port. With four parked connections for each of at most two credential ids, the idle pool uses at most eight connections under the 1,023-connection cap. Sources: [v52.0 vsock documentation](https://github.com/cloud-hypervisor/cloud-hypervisor/blob/v52.0/docs/vsock.md) and [v52.0 Unix muxer](https://github.com/cloud-hypervisor/cloud-hypervisor/blob/v52.0/virtio-devices/src/vsock/unix/muxer.rs).

The v52.0 API-only sequence is `cloud-hypervisor --api-socket path=<path> --seccomp true`, followed by `ch-remote --api-socket <path> create <VmConfig JSON>` and a separate `ch-remote --api-socket <path> boot`. `vm.create` stores configuration and creates the console devices; vCPUs begin only in `vm.boot`. The JSON uses the pinned Rust `VmConfig` field spellings, including `boot_vcpus`, `landlock_enable`, and serial `{ "mode": "Socket", "socket": ... }`. This is the causal boundary used by the slice 002 supervisor. Sources: [v52.0 command entry point](https://github.com/cloud-hypervisor/cloud-hypervisor/blob/v52.0/cloud-hypervisor/src/main.rs), [v52.0 VM configuration](https://github.com/cloud-hypervisor/cloud-hypervisor/blob/v52.0/vmm/src/vm_config.rs), and [v52.0 request handler](https://github.com/cloud-hypervisor/cloud-hypervisor/blob/v52.0/vmm/src/lib.rs).
