...
No runner runs `boot` today. A standard GitHub runner does carry `/dev/kvm` world read-write, measured on 2026-09-08, so the missing premise is not virtualization: it is the systemd user manager [`../decisions/ADR-0097-the-transient-user-service-owns-the-vm-lifetime.md`](../decisions/ADR-0097-the-transient-user-service-owns-the-vm-lifetime.md) gives a VM's lifetime to. The `boot-experiment` job in the workflow is what tests that premise, and it carries `continue-on-error` and is absent from the gate's `needs`, so it holds no merge. Two consecutive green runs and the premise verified are what would promote it.
