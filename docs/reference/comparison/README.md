...
| [Host environment is deny-by-default](./scenarios.md#host-environment-is-deny-by-default)                                 | ✅ yes                                                 | [✅ yes](./scenarios.md#environment-flake-pilot)         | [⚠️ partial](./scenarios.md#environment-glaipnir)          | ✅ yes                                              |
| [Session sockets refused as a source](./scenarios.md#session-sockets-refused-as-a-source)                                 | [✅ yes](./scenarios.md#session-sockets-vivarium)      | [➖ n/a](./scenarios.md#session-sockets-flake-pilot)     | ❌ no                                                     | ❌ no                                               |
| [Use an SSH key without the key entering the sandbox](./scenarios.md#use-an-ssh-key-without-the-key-entering-the-sandbox) | [✅ yes](./scenarios.md#key-material-vivarium)         | [❌ no](./scenarios.md#key-material-flake-pilot)         | [❌ no](./scenarios.md#key-material-glaipnir)             | [❌ no](./scenarios.md#key-material-podman)         |
...
| [Credentials scoped per tool](./scenarios.md#credentials-scoped-per-tool)                                                 | [❌ no†](./scenarios.md#per-tool-credentials-vivarium) | [❌ no](./scenarios.md#per-tool-credentials-flake-pilot) | [✅ yes](./scenarios.md#per-tool-credentials-glaipnir)    | [❌ no](./scenarios.md#per-tool-credentials-podman) |
...
The guest is an artifact somebody produced. These rows ask how that artifact is defined and assembled, whether the definition travels to another person and repeats there, what ran while it was being produced, and how you get one in the first place.
...
Read the bare ones first: they are what a comparison written by the subject would have left out. Three `†` cells cite a binding invariant; the fourth, `Credentials scoped per tool`, cites a design position no invariant states yet, logged as `Q-031` in [`open-questions.md`](../../plan/open-questions.md).

Ten of the twenty-five capability rows, and four of the six backend rows, were first found in a project other than vivarium.
