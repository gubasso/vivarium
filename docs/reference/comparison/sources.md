...
This repository, branch `develop` at `e1b3c73`.[^vivarium]
...
[^vivarium]: Verified 2026-08-18 at `ceb0027`. Partially re-read 2026-08-22 at `162f230`, where `[[workspaces]]` replaced the derived workspace: the workspace-mount, host-path, and mount-choice rows only. Re-read again 2026-08-26 at `e1b3c73`, after slices 031, 032, 025, and 027 closed, per the per-slice cadence below: the rollback, reclaim-disk, definitions, and instances rows, the lifecycle and agent-channel entries of the sweep, and the walkthrough paragraph that rested on them — the readings that moved when the `viv generations` family, `viv gc`, `start --generation`, `viv status -g`, `viv stop --all`, the stop ladder's first rung, both lifecycle `--json` records, and the declared `credentials.agents` channel landed. Every other row still carries the reading its own footnote names, and the pin moves so a later reader compares against the tree the tables now describe.
