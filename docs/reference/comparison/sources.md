...
<https://github.com/OSInside/flake-pilot> at `920f41e`, which was `main` at the last reading. Rust, MIT licensed. Both microVM routes are read at that one revision. Read at both of its microVM routes, because upstream publishes a `claude` registration for each and [the methodology](./methodology.md) fixes an isolation class rather than an engine.[^flake-pilot]
