...
No, and deliberately: the guest is a musl Alpine carrying the agent and the two helper binaries, with no Nix, no direnv, and no version manager, and upstream's own reasoning for keeping it that way is that installing the toolchain in the container defeats the purpose. The project's own description is visible at `/workspace` and does nothing. What answers the underlying need instead is [passthrough](./host-toolchain.md#bunkerbox), which reaches the host's toolchain rather than activating the project's — a different capability, scored at its own row and paid for at [the row beside it](./build-inside-boundary.md#bunkerbox).
