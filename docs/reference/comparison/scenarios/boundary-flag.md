...
Yes: the runtime is the literal `io.containerd.kata.v2` in `src/kata.rs`, and no flag reaches it. The one global flag that sounds like it might, `--workspace share|clone`, selects how the project tree is presented and leaves the kernel boundary alone. What a flag cannot weaken here, a file can — see [the file row](./boundary-file.md).
