...
No: `sci` runs the one `run=` command and then reboots, so the single execution slot is the application itself. That is the same in-guest emptiness that loses flake-pilot [a second session](./concurrent-sessions.md#flake-pilot).
