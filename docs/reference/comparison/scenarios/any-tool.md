...
vivarium `ceb0027`, 2026-08-18. Yes: vivarium is application-agnostic and classifies nothing inside as trusted or untrusted. The manifest names packages and mounts; it never names an application the tool holds an opinion about. That generality is the same position that loses vivarium the [credential-scoping row](./per-tool-credentials.md#vivarium), where knowing which directory belongs to which application is exactly what would be needed, and it is logged as `Q-031` in [`open-questions.md`](../../../plan/open-questions.md).
