...
No, and the encryption it does have is why the answer is worth stating rather than assuming. `encrypt` seals credential files at rest between runs, but everything about that arrangement is machine-local: the files live in the tool's persisted home under the user's own state, the passphrase is typed at start or exported into the environment, and no key travels. Nor could the project ask for it — `encrypt` is one of the three keys `.bunkerbox/project.conf` may not override, so the list belongs to whoever built the package rather than to the repository. A second person receives no secret with the project and authenticates the tool again. What the mechanism does answer is [the at-rest row](./credentials-at-rest.md#bunkerbox), which is a different question.

[^read]: Read at `vivarium` `ceb0027` on 2026-08-18; `flake-pilot` `44e3ab2` on 2026-08-20; `glaipnir` `8c7420e`, read 2026-08-20 — a later revision than the `21ef389` the rest of this subject is pinned to, read fresh for this row rather than inferred from the earlier one; `bunkerbox` `b7f14f3` on 2026-08-25.
