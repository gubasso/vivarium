...
Membership is decided per option by what depends on the value, not by the `vivarium.*` prefix ([`../../decisions/ADR-0041-resource-and-volume-channel-classification.md`](../../decisions/ADR-0041-resource-and-volume-channel-classification.md)). `vivarium.volumes` is the exception that proves it: a volume's guest mountpoint is guest system configuration, so it belongs to the build channel and adding a volume rebuilds. Only its host image path and virtual size resolve at launch. A manifest's `[[workspaces]]` rows are the second such case and reach Nix through no option of their own: `viv` expands each one when it resolves the manifest and compiles it into `vivarium.mounts` with its `target` equal to its `source`, because the tree is mounted at the path the host holds it at and the guest system therefore depends on that path ([`../../decisions/ADR-0110-the-workspace-is-an-ordinary-mount.md`](../../decisions/ADR-0110-the-workspace-is-an-ordinary-mount.md)). Moving a declared tree rebuilds; the rows a shared layer contributes to `vivarium.mounts` stay launch-channel and do not. `vivarium.credentials.agents` splits the same way as a volume: the guest agent is built to serve the declared ids, so declaring one rebuilds, while the host socket each id names resolves only at launch ([`07-secrets-and-config-sharing.md`](./07-secrets-and-config-sharing.md)).
...
- The build must be pure. No value the tool reads off the host may enter it. What a manifest declares is not such a value: a `[[workspaces]]` row is authored text, and the path it expands to is a build input by design ([`../../decisions/ADR-0110-the-workspace-is-an-ordinary-mount.md`](../../decisions/ADR-0110-the-workspace-is-an-ordinary-mount.md)), which is why one manifest can evaluate to different store outputs on two machines while the build itself reads nothing.
