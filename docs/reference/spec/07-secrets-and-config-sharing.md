...
Each host source resolves at launch and nowhere earlier. `ssh` resolves from `$SSH_AUTH_SOCK` and nothing else. `gpg` resolves by asking the GPG installation itself — `gpgconf --list-dirs agent-extra-socket` — because the restricted extra socket has no environment variable and its location is the agent's to name; a missing `gpgconf` or an empty answer is the same fault as an unset variable. That query returns a filesystem path and no secret, which is what keeps it on the right side of the provider-hook prohibition below: vivarium asks where a socket lives, never for a value. A declared channel whose resolved source is unset, missing, not a socket, or not owned by the user refuses before build and boot with `78` ([`14-exit-codes.md`](./14-exit-codes.md)), naming which fault tripped, because a relay that is missing rather than refused is discovered as an authentication failure inside the guest.

A relay ends when either side stops sending and that end of stream reaches the other, and the host MUST then open a replacement connection so the slot returns to the parked pool. The transport MUST therefore carry a half-close: a guest that closes its sending side MUST become an end of file at the host peer, and the reverse. Without that signal a finished relay is indistinguishable from an idle one, both ends wait, and the pool drains to nothing after as many clients as it has slots — which is a real defect vivarium has already been bitten by, recorded as [`../known-issues/resolved/KI-0002.md`](../known-issues/resolved/KI-0002.md). This requirement is on the transport, not only on vivarium's own code, so it binds the choice of backend version.
