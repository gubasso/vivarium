...
5. Ensure running — boot the VM if one is not already up for this manifest, injecting every declared workspace host path at launch (N5) and mounting each read-write at the same absolute path it occupies on the host (N16, [`06-workspace-and-project-environment.md`](./06-workspace-and-project-environment.md)). Host-derived resource ceilings are resolved and applied at this moment for the same reason the workspace paths are (N3, N19), and every process launched for the VM is placed in the target's host resource scope. It then waits for the guest to answer the control-socket liveness handshake specified in [`12-exec-and-shell.md`](./12-exec-and-shell.md), which is what makes a boot timeout detectable as `69` ([`14-exit-codes.md`](./14-exit-codes.md)).
...
`exec` and `shell` call the same ensure-running routine. When the control-socket ping and boot record prove the VM is already running for the same manifest, target, build, and workspace map, they skip preflight/build/boot and attach a session. When they must cold-start, they run the same hard preflight subset as `start` before any build or launch.
