...
- built — a sandbox output exists in the store but no VM is running. A clean `viv stop` lands here; this is the "stopped" state.
- starting — the VMM has been launched but the guest is not yet live (the control-socket liveness handshake does not answer yet). Transitional. Because `start` returns only once the guest answers (see What `viv start` does), this state is observable to a concurrent `viv status` while another process holds the per-target `flock` and is still booting ([`12-exec-and-shell.md`](./12-exec-and-shell.md)) — never in the window after your own `start` returned.
...
- failed — vivarium cannot treat the VM as healthy: boot failed, the VMM or guest exited abnormally, or a runtime record is broken. The specific cause is carried as a reason (`crashed`, `boot-timeout`, `socket-lost`, …), not a separate state.
...
4. Evaluate and build the outer flake to a store output. Identical inputs reuse the cached output (N4); the result is recorded as a generation ([`11-generations-and-build-history.md`](./11-generations-and-build-history.md)). Before anything boots, the selected build's launch contract schema — published by the build at `share/vivarium/launch-contract-schema` — is read against the running binary's own; a build that speaks another number is refused with `78`, both versions named (see Freshness and staleness below).
5. Ensure running — boot the VM if one is not already up for this manifest, mounting every declared workspace read-write at the same absolute path it occupies on the host (N16, [`06-workspace-and-project-environment.md`](./06-workspace-and-project-environment.md)). Host-derived resource ceilings are resolved and applied at this moment because they are launch-channel and a workspace path is not (N3, N19), and every process launched for the VM is placed in the target's host resource scope. It then waits for the guest to answer the control-socket liveness handshake specified in [`12-exec-and-shell.md`](./12-exec-and-shell.md), which is what makes a boot timeout detectable as `69` ([`14-exit-codes.md`](./14-exit-codes.md)).

Post-condition. `viv start` without `--attach`, exiting `0`, guarantees the VM was running at the moment it returned — never still `starting`. The guarantee is point-in-time, not durable: a guest that crashes immediately afterwards makes `failed` a correct next reading. `--attach` is excluded because it returns when the console stream ends rather than when the VM is ready (see Attach vs detach).
...
`exec` and `shell` call the same ensure-running routine. When the control-socket ping and boot record prove the VM is already running for the same manifest, target, build, and declared workspace set, they skip preflight/build/boot and attach a session. When they must cold-start, they run the same hard preflight subset as `start` before any build or launch.
...
`--no-rebuild` and `--generation <n>` keep old artifacts reachable on purpose, which is why every boot path reads the selected build's launch contract schema from the built output itself — never from a compile-time constant — before executing anything in it. A build whose number differs from the running binary's, or that predates the schema's publication, is refused before boot with `78` under `vm.launch-contract-skew`, naming both versions; `viv start --rebuild` rebuilds with the running version, and running the vivarium generation the build was made by is the other exit ([`../../decisions/ADR-0102-the-installation-supplies-vivarium.md`](../../decisions/ADR-0102-the-installation-supplies-vivarium.md)). The refusal comes before the boot record is written, so a refused boot never changes what `status` reads as the running VM's provenance. A launch or boot record written by another vivarium version is diagnosed the same way — `78`, both schema versions named, distinct from the unreadable-record case — and `viv status` reports the skew beside `running` rather than refusing, because the state is data.

## Attach vs detach

By default `start` boots the VM as a background resource and returns once the guest answers — the VM is a persistent thing `exec`/`shell` connect to, and the wait is what lets the post-condition above hold. `--attach` instead streams the VM console until it exits; `Ctrl-C` detaches the console and leaves the VM running — it never stops the VM. The stream is the same capture that feeds `console.log`, so attaching works against a VM that is already running ([`16-logging-and-diagnostics.md`](./16-logging-and-diagnostics.md)). Because an attached `start` returns when the stream ends, a guest that powers itself off leaves `--attach` returning with no VM, which is why the post-condition excludes it. Detaching is the attached form's designed ending, so `Ctrl-C` exits `0`; the other terminating signals report `128+S` the way a session does ([`12-exec-and-shell.md`](./12-exec-and-shell.md)).
...
1. Agent shutdown — ask the in-guest agent over the control socket to begin an orderly guest shutdown ([`12-exec-and-shell.md`](./12-exec-and-shell.md)), and on its acknowledgement wait for the VM to exit on its own. `--timeout` bounds this orderly rung (default 10 s; `-1` waits indefinitely): the grace is the time a slow guest is allowed to finish committing, which is what the flag exists for.
2. Backend soft-off — when the agent cannot be reached (no answer, a protocol error from an agent that predates the request, or a boot record this version cannot read), fall back to the backend's ACPI-class power signal under the operator's grace, restarted rather than reduced: the ask's own bounded cost — at most its two-second budget, and only when the agent held the connection without answering — is not deducted, because the power path's compiled transaction has a floor the default grace only just clears, and a fallback squeezed below its floor fails a stop it was specified to complete. How long the power signal itself is given before the VM is destroyed is a compiled constant of the supervisor, never the operator's flag — patience does not make an unreachable agent reachable, so there is nothing there for a longer grace to buy.
3. Hard poweroff — when the grace expires, pull the power. One case earns a bounded reprieve first: a guest that acknowledged the shutdown request and then exceeded the grace inside its own shutdown transaction is handed to the power path with its compiled window — a bounded overshoot of the deadline, accepted because only the supervisor's cancellation path both destroys the VM and sweeps the runtime directory — and the group is killed if even that window passes. `--force` skips the orderly rung entirely and pulls the power at once (possible data loss); it conflicts with a nonzero `--timeout` — that combination is a usage error.

`viv stop --all` applies the same ladder, with the same flags, to every sandbox with retained state that is up, enumerated through the same derived index `viv status -g` reads, and needs no selected manifest — the deliberate exception to the working-directory rule, because ending a day starts wherever the operator is standing. Nothing running is a no-op that exits `0`. Every enumerated sandbox is attempted rather than the sweep stopping at the first fault — stopping early would leave the rest running behind a nonzero exit — each failing sandbox is named on stderr as it fails, and the first failure is the sweep's result ([`14-exit-codes.md`](./14-exit-codes.md)): a sweep never reports success for a sandbox it could not confirm stopped.
...
`destroy` never touches a workspace, the config root, the selected manifest, or store contents. Unlinking the GC roots only makes store paths reclaimable — they are actually freed by a later `viv gc` ([`11-generations-and-build-history.md`](./11-generations-and-build-history.md)). The unlink happens after the VM is down, never beside a running one: a generation's root is what keeps a running guest's own closure out of a collection's reach, which is why `viv generations prune` refuses outright where `destroy` merely orders itself correctly ([`06-workspace-and-project-environment.md`](./06-workspace-and-project-environment.md)). Because the build is reproducible from the manifest, `destroy` followed by `start` costs at most a rebuild; the only irreversible loss is volume data, which is why removal is guarded by the prompt and spared by `--keep-volumes`. `destroy` is idempotent — nothing to tear down exits `0`.
...
The admission check (step 3 above) is a separate gate, not a catalog probe: preflight asks whether this host can run a VM at all, admission asks whether it can run another one right now. Its thresholds and its warning are owned by [`17-resources-and-capacity.md`](./17-resources-and-capacity.md); `viv doctor` reports the same host readings as ordinary soft checks.
...
`start`'s result is a side effect (a booted VM), so on success stdout is empty; all progress and status go to stderr. `stop` and `destroy` follow the same rule — empty stdout on success, one record under `--json`, whose shapes [`01-command-surface.md`](./01-command-surface.md) fixes. The full convention is in [`ADR-0015`](../../decisions/ADR-0015-cli-output-and-failure-contract.md).
