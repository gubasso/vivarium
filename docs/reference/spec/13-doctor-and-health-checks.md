...
| `state-files-parse`        | config         | the project registry or the identity index cannot be read or does not parse. The message names the file **and which of the two faults tripped** — a genuine I/O fault or malformed content — because the repairs are `chmod` and an editor respectively ([`02-config-and-xdg-layout.md`](./02-config-and-xdg-layout.md))                                                                                                                                                                            |
...
`state-files-parse` is **soft** for the reason `config-parses` is: the read path already fails closed at the point of use with the precise code — `74` for a genuine I/O fault, `78` for malformed content ([`02-config-and-xdg-layout.md`](./02-config-and-xdg-layout.md)) — so the check buys early visibility rather than a second gate, and it is the answer to a `doctor` run that would otherwise report a healthy host to a user whose `viv status -g` just exited `78`. Promoting it would also overstate the fault: a malformed **registry** does not by itself block a launch, because `--manifest` overrides the binding without reading the file at all. Being soft keeps it out of the preflight subset, whose codes are `69` / `77` / `78` and could not carry a `74` honestly.
