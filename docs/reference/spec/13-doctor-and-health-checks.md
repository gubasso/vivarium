...
`kvm-device-present` and `kvm-device-accessible` are split because remediation differs: enable virtualization in firmware vs join the `kvm` group.
...
- **VMM (Cloud Hypervisor).** Launched unprivileged with `PR_SET_NO_NEW_PRIVS`, capabilities dropped toward zero, built-in seccomp enabled (`--seccomp true`), a Landlock path allowlist where the kernel supports it, and cgroup limits.
- **virtiofsd (one process per share).** Launched unprivileged, `--sandbox=namespace`, seccomp on, `cache=none`, with only its own share writable — never as root and never `--sandbox none`, the configuration behind the guest-root-to-host-root escape CVE-2026-47243.
