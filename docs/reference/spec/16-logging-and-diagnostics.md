...
The default log is bounded — it rotates by size and keeps a small number of prior files so it never grows without limit. The exact size and count are an implementation detail, not a stability promise; only the bounded guarantee is normative.
...
- Third-party output. `nix` and the backend write to their own error streams. Vivarium classifies and elides what it captures; it cannot guarantee anything about a byte stream produced by another program.
