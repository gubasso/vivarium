...
Secrets and personal paths in log records are an open follow-up (a later ADR): redaction, when specified, applies to records **before** they are written, so no unredacted copy exists on disk. See [`07-secrets-and-config-sharing.md`](./07-secrets-and-config-sharing.md) for the secret-handling model the redaction rules will extend.
