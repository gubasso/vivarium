...
Exit codes for `viv volume list` / `rm` follow the per-command matrix in [`14-exit-codes.md`](./14-exit-codes.md) — notably `75` when `rm` refuses because the VM is still running (stop first).
