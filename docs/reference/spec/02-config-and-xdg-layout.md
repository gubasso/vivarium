...
- State root — per-sandbox runtime state the tool writes: the generation profile that retains builds ([`11-generations-and-build-history.md`](./11-generations-and-build-history.md)), the boot-provenance record, persistent volumes, and the diagnostic log (`logs/vivarium.log`, written by default — see [`16-logging-and-diagnostics.md`](./16-logging-and-diagnostics.md)). Each sandbox directory is keyed by its manifest name.
...
Scanning is accelerated by `$XDG_CACHE_HOME/vivarium/workspace-index.json`. The index stores only manifest names, resolved manifest paths, modification times, a hash of each manifest's own bytes, and unexpanded workspace source tokens. The hash is what makes "a changed library snapshot" mean the bytes rather than the metadata: a rewrite that preserves the modification time MUST still invalidate the index, because a manifest that has since claimed the invoking directory would otherwise never become a candidate and ownership would resolve without reporting the second claim. It is not a cryptographic digest and MUST NOT be relied on as one; a hash that disagrees with a previous run's is a cache miss, which this cache can afford. Every field is recomputable from the manifest library. A missing, malformed, or stale index is a cache miss: vivarium rebuilds it before resolving ownership. A changed library snapshot invalidates it. The selected manifest's current text is checked again before its ownership is trusted, so the cache narrows candidates and never becomes authority.

No state-root file has a role in selection. An older version wrote `registry.toml` and `identity.toml` beside their `.lock` files, and it keyed each project's state on a minted project id rather than on a manifest name. If any of the four files remains, vivarium neither reads, writes, nor deletes it; a `projects/<id>/` root retained from that keying is reported by the soft `state-manifest-orphans` doctor probe and deleted by nobody, which is the same report-rather-than-reap judgment the rest of this page applies. Removing any of them is an operator decision and costs a rebuild of nothing. There is no project-local binding or identity file, and no command writes inside a workspace (N9 and N13).
...
A writer that dies between creating its temporary file and renaming it leaves that file behind. Nothing removes it and nothing reports it, so the cache root accumulates one such name per crash. This is a known consequence rather than a specified behaviour: it costs disk and never correctness, because no reader ever opens a name it did not rename into place.
...
The target component exists now so adding another target later does not relocate state. State and runtime layouts use it symmetrically, and the reason is that the two are read together: the ensure-running protocol resolves a runtime directory and its state directory from one pair of components, so a layout that spelled the pair differently on the two sides would make every such resolution carry a translation nobody can verify from either path alone. Multiple `exec` or `shell` sessions attach to one target rather than creating targets ([`12-exec-and-shell.md`](./12-exec-and-shell.md)). This directory holds the generation profile, the boot-provenance record, and persistent volumes (`volumes/<name>.img`, always including `default`). Volumes live under state, not cache, because their contents are user data and not regenerable. Generation layout and lifecycle are specified in [`11-generations-and-build-history.md`](./11-generations-and-build-history.md); the volume model in [`06-workspace-and-project-environment.md`](./06-workspace-and-project-environment.md) and [`../../decisions/ADR-0019-volume-model.md`](../../decisions/ADR-0019-volume-model.md).

Two records answer two different questions, and exactly one answers each. What a project last built is the generation profile's `current` pointer — a retention record, under the state root because a run produces it, whose numbered symlinks are the garbage-collector roots that make the answer durable. What the running VM was booted from is `running-build`, a store path as text beside the profile — a boot-provenance record, written just before the launcher runs, which `status` compares against `current` for freshness and `viv generations prune` reads as the boot record ([`../../decisions/ADR-0085-a-running-guest-pins-the-store-paths-it-reads.md`](../../decisions/ADR-0085-a-running-guest-pins-the-store-paths-it-reads.md)). The two may name different outputs — `viv start --generation <n>` boots a build `current` has moved past — which is why the guard never reads `current`. `running-build` lives here rather than in the runtime directory because the supervisor's cleanup sweep removes only its own allowlist and aborts on anything else.
