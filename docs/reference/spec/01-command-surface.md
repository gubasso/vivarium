...
- `viv manifest list` — `--json` emits `{ "manifests": [ { "name", "path", "image", "pieces" } ] }`, one row per defined manifest (not the bound one — that is `viv config`). `pieces` is the ordered piece list; `image` and `pieces` are included so the composition is visible without a follow-up `show`. Empty/absent library emits `{ "manifests": [] }` and exits `0`.
...
- `viv status -g` / `--global` — enumerate every project in the state registry ([`02-config-and-xdg-layout.md`](./02-config-and-xdg-layout.md)). `-g` is scoped to `status`, not a global flag — it is the only flag that reports across projects; the one other cross-project form, `viv stop --all`, acts rather than enumerates and keeps its own spelling. `--json` wraps the list under a named key, matching `images`/`manifests`, and carries the same `resources` / `runtime` split per row:
  ...
  `path_missing` is `true` when the bound directory no longer exists; such a row also reports `state: "absent"`. The two flags are unrelated and easy to confuse: `stale` means a running VM is behind its build, `path_missing` means the project directory is gone.
