...
| `viv trim [--json]`                                                            | Reclaim both resources in one invocation: the memory rung first, then the volume rung. A convenience over `viv memory trim` and `viv volume trim`, carrying neither's flags — an operator with a target or a single volume in mind names that command instead. It never fails fast, and a run that ends in failure reports no record, so a script that needs both figures under partial failure calls the two commands separately ([`../../decisions/ADR-0113-a-reclaim-verb-sits-under-its-resource.md`](../../decisions/ADR-0113-a-reclaim-verb-sits-under-its-resource.md)). Requires a running VM (`75` otherwise). See [`17-resources-and-capacity.md`](./17-resources-and-capacity.md).                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `viv gc [--json]`                                                              | Run the store garbage collector — a global, whole-store sweep that reclaims store paths unreachable from any GC root, vivarium's or not. Project-scoped retention lives in `viv generations prune`. See [`11-generations-and-build-history.md`](./11-generations-and-build-history.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `viv memory trim [--to <MiB>] [--json]`                                        | Reclaim memory the project VM is holding but no longer needs, and report what the host got back. Bounded, synchronous, and always explicit — vivarium never reclaims from a running guest on its own (N23). `--to` names the figure the guest is asked to reach; without it the target is the measured working set with headroom. Requires a running VM (`75` otherwise). See [`17-resources-and-capacity.md`](./17-resources-and-capacity.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
- stdout carries the result only — a human table/line for data commands (`images list`, `manifest show`, `generations list`, `volume list`, `config`/`config sources`/`config eval`, `status`, `doctor`'s report), a `--json` record in machine mode, and nothing for side-effect commands whose result is a VM state change: `start` prints nothing in either mode — except `--attach`, whose stdout is the raw console stream, which is why the pair `--attach --json` is refused as usage (`64`): a byte stream has no record to promise — while `stop` and `destroy` keep the empty human face and emit their one record under `--json` (Lifecycle output below, enacting [`10-vm-lifecycle.md`](./10-vm-lifecycle.md)'s one-record sentence). `memory trim`, `volume trim`, `trim`, `volume prune`, `generations prune`, `generations activate`/`rollback`, and `gc` are the exception that proves the rule: they act, but what a user runs them for is the result they return — a measurement, the rows unlinked, or the moved pointer — so they print it (shape below).
...
`viv memory trim`, `viv volume trim`, and the `viv trim` fan-out over both mutate, but their result is a measurement rather than a state change ([`17-resources-and-capacity.md`](./17-resources-and-capacity.md)), so they print it. All three follow the conventions above: a single keyed JSON object, no `schema_version`, `_bytes` on every measured quantity, and a before/after pair so a consumer never has to trust a delta it cannot check.

- `viv memory trim [--to <MiB>]` — human output is one line naming what the host got back. `--json` emits one record:
  ...
  The before/after pair is admissible here where [`../../decisions/ADR-0082-guest-memory-return-is-measured-on-the-backing-object.md`](../../decisions/ADR-0082-guest-memory-return-is-measured-on-the-backing-object.md) rejects one, because that record's objection is to an unattended pair: reclamation is asynchronous, and two readings cannot see a transition they did not bracket. This command is the bracket. It drives the balloon itself — inflate to the target, hold until the charge stops moving under a bounded wait, restore the headroom — and reads the pair around exactly the one transition it caused. The metric is already the one that record demands: the scope charge moves only when the backing object's pages are actually freed (the reconciliation is recorded in [`17-resources-and-capacity.md`](./17-resources-and-capacity.md)'s Reporting section), so a settle that runs out of patience costs a smaller figure, never a false one. The series requirement stays with the elasticity verification it was written for.
...
- `viv trim` — the fan-out over both rungs. It takes no `--to` and no volume name: a target or a single volume is a reason to name the specific command. `--json` nests one subtree per resource, each verbatim the record its own command emits, with the sandbox key hoisted:
  ...
  There is deliberately no top-level `reclaimed_bytes`. The two subtrees measure different quantities — host memory the hypervisor process was holding, and blocks a sparse image occupies — so a grand total would be a figure with no unit and no pair behind it, which is exactly what the before/after convention exists to prevent. A consumer that wants one number reads the subtree whose resource it cares about. Both subtrees are always present, because a record is emitted only when both rungs succeeded.

  The memory rung runs first: it is the only reclaim N23 leaves the user (`17-resources-and-capacity.md`), while the guest trims volumes on its own schedule. The fan-out does not fail fast — a failing rung does not skip the other, and each failure is named on stderr as it happens. Partial failure then follows the rule `viv stop --all` carries (Lifecycle output below): the first failure's category is the exit code, its envelope is the machine-readable result, and no record is emitted. The measurement a succeeded rung already took is therefore lost in machine mode, and only there — the human face prints each rung's line as that rung completes, so a person sees what came back before the failure is reported. A script that needs both figures across a partial failure calls `viv memory trim` and `viv volume trim` separately ([`../../decisions/ADR-0113-a-reclaim-verb-sits-under-its-resource.md`](../../decisions/ADR-0113-a-reclaim-verb-sits-under-its-resource.md)).
...
- `viv generations prune` — unlinks retained generations rather than freeing store space, which a later `viv gc` reclaims ([`11-generations-and-build-history.md`](./11-generations-and-build-history.md)), so its record is the rows it unlinked rather than a byte measurement:
    ...
    "generations": [
      {
        "number": 3,
        "current": false,
        "store_path": "/nix/store/…-vivarium-vm",
        "manifest": "rust-web",
        "lock_digest": "sha256:…",
        "backend": "cloud-hypervisor",
        "built_at": "2026-08-25T14:10:58Z"
      }
    ],
    "kept": 2
  }
  ...
  The rows carry the same keys `generations list` publishes, so a `list` taken beforehand predicts this record exactly. `kept` counts the generations that remain, always including the one `current` points at, which is never a candidate ([`11-generations-and-build-history.md`](./11-generations-and-build-history.md) owns that rule and its reason). Nothing to unlink is `{ "generations": [], "kept": n }` at exit `0`.
...
Each row reports one public input — an input of the generated flake itself; a transitive node is not a row. `before` is `null` whenever the row's pin is newly created, which covers both the run that creates the lock and an input the existing lock did not yet carry — either is a success, not an error — and both sides are `null` for an input locked without a revision or hash, the base flake's own row being the ordinary case. `changed` is decided over the input's whole resolved subtree, not its own node alone: `viv update <image>` moving a component the base declares changes no public node's own pin, and a row that read only itself would report a moved lock as unmoved. Naming inputs restricts the rows to those inputs plus any public input whose pin moved by following one of them; a requested row is always reported, `changed` false included. A repeated name is one request. An unknown name is `64`, decided by vivarium — Nix answers it with a warning and a successful no-op, which would read as "nothing moved". When the base carries its own `flake.lock`, the run reports on stderr that the pins that file holds re-seed every re-resolution and move only when it does ([`../../decisions/ADR-0112-the-selected-image-carries-the-base-flake.md`](../../decisions/ADR-0112-the-selected-image-carries-the-base-flake.md)).

An `<input>` is one of vivarium's baseline input names — `nixpkgs` and `microvm`, the rows above — the identifier a shared image or piece declares in its `inputs.toml`, or the selected image's own name when it carries a base flake ([`03-artifact-model.md`](./03-artifact-model.md), [`../../decisions/ADR-0112-the-selected-image-carries-the-base-flake.md`](../../decisions/ADR-0112-the-selected-image-carries-the-base-flake.md)). All three share one namespace, which is why an artifact may not declare a baseline name or the base's; the string a user passes here is the same string the artifact wrote and the same `name` this record reports ([`../../decisions/ADR-0074-declared-inputs-are-pinned-by-the-effective-lock.md`](../../decisions/ADR-0074-declared-inputs-are-pinned-by-the-effective-lock.md)). A followed baseline is requested by its own name; vivarium invokes Nix on the base input owning its node, and the requested name stays the reported row. Updating does not build: the pin has moved and the next `start` produces the generation that records it ([`11-generations-and-build-history.md`](./11-generations-and-build-history.md)).
