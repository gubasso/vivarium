...
## The sibling script: `tests/host/update-check`

The slice 018 acceptance: as the user, redirect the guest's build inputs through your own base flake, move a component's pin with `viv update`, rebuild, and read the moved component from inside the booted guest — no vivarium release in the path, no unnamed public pin moved. A separate script because the claim is a booted guest's contents across a pin move, which needs `/dev/kvm`, a systemd user manager, and two real builds.

```console
$ tests/host/update-check
```

The method worth knowing before reading a result. The movable component is a local `git+file:` flake the base declares, so upstream movement is a commit and every reference resolves offline; the baselines are pinned from this repository's own lock through `nix flake archive` store paths, written into the base flake itself rather than into the environment, because the base owning the redirect is the claim under test. The component surfaces as `/etc/dep-text` through the base's own outputs, so "the guest carries the moved component" is read with `viv exec` from inside the guest, never inferred from the lock — and the negative is asserted first: after the upstream moves and before `viv update`, a stop and start still boot the old component, which is N3 doing its job. The whole move repeats with a third revision, and the unmoved `nixpkgs` node is asserted byte-identical across the named update. `--clean` removes retained scratch roots; `VIVARIUM_UPDATE_TEMP_BASE` overrides where they go.
...
### The stop ladder's first rung works end to end, and its evidence is a floor the fallback cannot beat

Measured 2026-08-25 on the same host, landing slice 027. The shutdown request pair reached a real guest: with the backend's API socket deleted out from under a running VM — so the ACPI path could not act at all — `viv stop` completed cleanly with its record reading `agent`, inside a stop that finished under the eight-second floor the degraded power path cannot get under (the supervisor's six-second power-button window plus its two-second destroy window), and an unsynced guest write survived the rung. The same trial's second leg deleted the control socket instead and the stop fell through to `power-signal`, still exiting `0`. The distinguishing evidence is the floor, not the record alone: a record is the product's own claim, while a sub-floor clean stop with the power path physically unavailable can only have gone through the guest.

The grace acceptance measured as designed on a guest whose every orderly shutdown takes twenty seconds by construction (a root oneshot whose `ExecStop` sleeps): under `--timeout 40` the stop ended through the agent rung after more time than the default grace permits, and under `--timeout 3` the acknowledged-then-wedged shutdown was handed to the power path's compiled window and the VM was down in well under the twenty seconds its own transaction wanted — the escalation observably cut it short, with cleanup intact and exit `0`. The sweep stopped two running sandboxes from a directory no manifest declares, reported both rows `built` with their rungs, answered a second invocation with an empty record, and the unsynced write in one guest survived the sweep. One trial run each, then the whole `pre-push` profile repeated them.

### A relative path input is a layer and an absolute one silently pins, which decided how a user's base flake joins the build

Measured 2026-08-26 on the target host with `VIVARIUM_HEAVY_DRIVE` set, on Nix 2.34.8, shaping slice 018 before any code. Two fixture rounds under `tests/host/heavy-run`, offline by construction: every input was a local `path:` or `git+file:` fixture, so no run named a registry or a forge. The scripts and full output are session artifacts; what follows is everything load-bearing.

The figures. The production first-pin path works over a mutable `path:` input: `nix eval` with no `--no-update-lock-file` wrote the lock, and `nix flake lock` — despite setting `failOnUnlocked` where `eval` and `update` do not — accepted it too, with the node locked as `lastModified`, `narHash`, `path`, `type`. But an absolute `path:` node behaves opposite to the assumption the slice was shaped under: after an in-place edit of the input directory, evaluation against the stale lock returned the old content silently — exit `0`, old value, even under `--no-eval-cache` — rather than failing on a hash mismatch. There is no staleness error to classify; the edit is invisible until the pin moves. A relative `path:./images/b` input locks as `path` plus `type` only — no content hash, no timestamp — survives republication of the tree with fresh mtimes, and serves an in-place edit on the next evaluation, while the inputs the base itself declares are still pinned as ordinary root-lock nodes. That is layer semantics for the text and lock semantics for the references, and it is what settled the rendered form as `path:./images/<name>` against the copy the generated tree already carries: the absolute form was rejected for serving stale content silently, not for a portability inconvenience — though that too was observed, a traveled absolute lock failing on the machine that lacks the recorded path.

A dependency's own `flake.lock` seeds every generation of the parent lock, not just the first: a zero-name `nix flake update` (recreate) re-seeded the sub-lock's pin, and so did a named update of the path input carrying it; only deleting the sub-lock let resolution reach the branch head. And `nix flake update <name>` on a `follows` alias exits `0` with a warning — `'<name>' does not match any input of this flake` — moving nothing, while updating the input that owns the followed node moves it. Both are semantics `viv update` must own itself: translate a requested alias to the input that owns its node, and report that a base carrying its own lock holds its pins by that lock. The flag matrix behaved: `--output-lock-file` writes the output even when nothing changed, overwrites a pre-existing file, and leaves the in-tree lock untouched; `--reference-lock-file` naming a missing file is silently treated as absent, so the caller must pass it only when the lock in force exists.

Worth keeping beside the figures: combining `<input>.follows` with an override of that same input's inputs is not an error — Nix silently ignores the override and the lock shows the followed subtree intact — so the renderer must simply never emit the shape, and no diagnostic will catch it if it does. A raw space in a `path:` URL is misparsed into a store-relative path before it fails; percent-encoded forms all lock, but the chosen relative spelling makes the question moot because the image name grammar admits no such characters. `nix eval --file <flake.nix> --apply` reads a flake's `inputs` and its `outputs` function arguments without `--impure`, which is the probe form the composition seam uses, and `builtins.functionArgs` is what makes an implicit input — an `outputs` argument never declared under `inputs` — visible to it.

### A user's own base flake moved a guest component with no release in the path, read from inside the booted guest

Measured 2026-08-26 on the target host with `/dev/kvm`, a systemd user manager, `$XDG_RUNTIME_DIR`, and `VIVARIUM_HEAVY_DRIVE` set, on Nix 2.34.8. `tests/host/update-check`, first complete run: `PASS=7 FAIL=0 SKIP=0 RECORD=3`. This is the closing evidence for slice 018's end-to-end acceptance, landed the same day as the shaping findings above.

The figures. A first `viv update` on a project whose image carries a base flake created the pin in under a second and reported rows for `microvm`, `nixpkgs`, and the base, each `before: null`. The first `viv start` built and booted in 49s against a warm store, and the guest served the base-declared component (`/etc/dep-text` = `one`). The upstream then moved and nothing else happened: a stop and a start later the guest still served `one` — no ordinary build re-resolves. `viv update update-base` moved the component's pin under the base while the `nixpkgs` node stayed byte-identical, the row for the base read `changed: true` off its subtree fingerprint, and the rebuild booted in 22s serving `two`. The whole move repeated with a third revision and the guest served `three`.

Worth keeping beside the figures: the moved pin sits below a hashless relative `path:` node, so a row that compared only its own node's `locked` would have reported the moved lock as unmoved — the row's `changed` is computed over the input's whole resolved subtree for exactly this reason, and the lane's central check would have caught the regression. And the first update's row set names only public inputs: the component itself is a transitive node and never a row, which is the boundary `spec/01` draws between vivarium's input namespace and the lock's interior.
...
### The balloon reclaim returns dirtied page cache through the scope charge, bracketed by the command that asks for it

Measured 2026-08-27 on the target host, by `workflow_28_memory_trim_reclaims` twice in a row. A live guest read a few thousand store files to fill its page cache, and one `viv memory trim` — inflate the balloon to the guest-derived target, wait for the scope charge to settle, deflate — reported a positive `reclaimed_bytes` with `viv status` showing the same fall through the same reading, while a `setsid`-detached workload survived the operation. A second trim immediately after reclaimed nothing and exited `0`, which is the fact-not-failure shape spec/01 fixes. This is the first product-path consumption of the ADR-0082 metric: the harness's series sampler verified elasticity, and the command's own before/after pair is admissible because the command brackets the one transition it causes — the scoping is recorded in spec/01's reclamation-output section. The disk rung's chain needed no new measurement: `workflow_28_volume_trim_returns_blocks` reproduced the fstrim finding above on the home volume through the new agent bridge, returning most of a written-and-deleted 512 MiB to the sparse image, with the trim request acknowledged only after the root-owned unit's done marker — completion, not motion, because the host stats the image right after.
