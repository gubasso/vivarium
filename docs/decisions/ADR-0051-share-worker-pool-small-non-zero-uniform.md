...
Superseded by [`ADR-0096-the-share-worker-pool-takes-the-daemon-default.md`](./ADR-0096-the-share-worker-pool-takes-the-daemon-default.md) — 2026-08-05, on a concurrent measurement this record named as the open item and could not run. Every claim below stands as the reasoning for the value chosen here; what moved is the value.
...
The concurrent workload ran on 2026-08-05, and `4` did not survive it. Across `{1, 4, 16}` concurrent clients and two lane runs, no non-zero pool won a cell, and pool 0 scaled with the client count instead of stalling — so the head-of-line premise above is contradicted rather than merely unconfirmed. The successor named in `Status` carries the value from here.
