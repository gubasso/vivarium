...
[`../reference/spec/02-config-and-xdg-layout.md`](../reference/spec/02-config-and-xdg-layout.md) said the state root "holds logs" but left format, level, path, rotation, and flags undefined; ADR-0026 dropped `--log-format`, deferring it to a "future `viv logs` subsystem." A decision is needed now: where logs go, in what shape, and how the CLI controls them — without a `logs` command.
...
- Active-by-default file logging, no reader command; docs point to the path; existing `-v`/`-q` plus new file-only flags control it.
