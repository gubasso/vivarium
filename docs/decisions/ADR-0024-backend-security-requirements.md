...
Threat model: guest code is assumed adversarial — arbitrary execution, kernel-exploit attempts, data exfiltration. A shared host kernel is a single-bug boundary; the microVM boundary forces an attacker to chain a guest-kernel escape and a VMM/hardware break. To keep that second wall real, every admissible backend must provide:
