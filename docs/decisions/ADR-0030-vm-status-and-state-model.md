...
vivarium had no verb to report a project VM's operational state, and its lifecycle vocabulary ([`../reference/spec/10-vm-lifecycle.md`](../reference/spec/10-vm-lifecycle.md)) left `failed`/ `crashed` and a `starting` transitional undecided, and listed `stale` as a peer state. A `status` command needs a settled, closed enumeration to report against.
...
- A small vivarium-owned set: resting states plus `starting`/`stopping` transitional and one `failed` state, with staleness modelled as a flag.
