...
Enacted by the `/nix/store/.links` mask in [`nix/guest.nix`](../../nix/guest.nix), and measured on both boots of the persistence spike: a rooted collection completed in seconds, the lower layer's entry count was unchanged across it, and no descriptor exhaustion occurred.

Why the public prior art does not carry this. [`shazow/agentspace`](https://github.com/shazow/agentspace) runs the same store type over the same overlay, but its lower layer is microvm.nix's generated store image, which contains store paths and nothing else. vivarium shares the host's literal `/nix/store` ([`ADR-0038`](./ADR-0038-guest-store-sharing.md)), so `.links` is present in the lower layer here and absent there. The gap is a consequence of vivarium's sharing decision, not of the store type.
