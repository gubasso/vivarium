...
- The hypervisor (`cloud-hypervisor`), its control client (`ch-remote`), and every host-side filesystem daemon (`virtiofsd`) are members of the built runner's closure, pinned by the project's lockfile and recorded in the generation record beside the lock revision. None is ever resolved from `$PATH`.
