...
- Good: native type errors and module-system merge; the only bespoke check is the literal-path validation.
- Bad: reading launch data costs a pure evaluation per start; piece authors write Nix module syntax.
...
Amended by [`ADR-0110-the-workspace-is-an-ordinary-mount.md`](./ADR-0110-the-workspace-is-an-ordinary-mount.md) — a declared workspace compiles to `vivarium.mounts` and is build-channel, so a member of this option family may now be a build input. Host-side expansion for those rows happens when the flake is generated rather than at launch. Authored `[[mounts]]` are unchanged.
