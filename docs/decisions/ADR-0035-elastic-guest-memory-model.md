...
Amended by [`ADR-0113-a-reclaim-verb-sits-under-its-resource.md`](./ADR-0113-a-reclaim-verb-sits-under-its-resource.md) — 2026-08-27, the bounded, user-invoked reclaim this record is built around is spelled `viv memory trim`, and `viv trim` becomes a fan-out over it and the volume rung. The elastic model, the rejected controller, and the zero-size balloon stand exactly as recorded; only the spelling moved.

Enacted by slice [`028-memory-comes-back-without-a-stop`](../plan/slices/028-memory-comes-back-without-a-stop/README.md) — 2026-08-27, the bounded reclaim runs: `viv memory trim` inflates the balloon to a target derived from the guest's own report, restores the headroom in the same operation, and reports the scope-charge pair, demonstrated by `workflow_28_memory_trim_reclaims` returning dirtied page cache on a live guest.
