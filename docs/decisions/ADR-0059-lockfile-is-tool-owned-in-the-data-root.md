...
Amended by [`ADR-0112-the-selected-image-carries-the-base-flake.md`](./ADR-0112-the-selected-image-carries-the-base-flake.md) — the resolution that creates the tool-owned lock may be seeded by a base flake's own `flake.lock`, a file vivarium does not own and never writes. Everything else survives: the lock's home, its per-target scope, and that only `viv update` moves it.
