...
Amended by [`ADR-0110-the-workspace-is-an-ordinary-mount.md`](./ADR-0110-the-workspace-is-an-ordinary-mount.md) — a declared workspace joins `vivarium.volumes` on the build-channel side. That is the third application of the rule decided here rather than an exception to it: a workspace's guest path is the host path it mirrors, so a build output depends on it.
