...
[`ADR-0010-secrets-never-in-nix-store.md`](./ADR-0010-secrets-never-in-nix-store.md) named encrypted-at-rest one of two legal shapes a secret may take, but never said what vivarium _does_ about it. [`../reference/spec/16-logging-and-diagnostics.md`](../reference/spec/16-logging-and-diagnostics.md) meanwhile enumerated "anything read through the encrypted-at-rest channel" as secret-class, which presumes a channel vivarium reads through. Either vivarium builds one, or it states plainly that it does not.
