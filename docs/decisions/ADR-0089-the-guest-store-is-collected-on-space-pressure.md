...
The trigger fires on a real filesystem and its arithmetic is exact — but the collection frees nothing, measured 2026-08-05. `scripts/store-pressure-check --arm e`, on an image built with scaled thresholds and with upstream's free-space test hook removed, so every reading is real `statvfs` on real ext4. On the crossing iteration the daemon asked to free 754,675,712 bytes against 855,937,024 available and a 1.5 GiB target — exact to the byte, which confirms this record's "collect until `max-free` is free again" against a filesystem rather than against a hook.
