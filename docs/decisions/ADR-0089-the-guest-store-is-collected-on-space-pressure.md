...
**What it did not do is reclaim.** Guest free space did not recover and the dead-path count rose monotonically across the run and never fell. So this decision's _mechanism_ is confirmed and its _effect_ is not: as it stands, space-triggered collection does not bound the store volume in this topology. The cause is a stated hypothesis, not a measurement — the dead set is dominated by lower-layer paths a `local-overlay` store declines to delete, and `bytesFreed` is apparent size — and closing it needs the collector's per-path decisions instrumented, not another boot. Deliberately **not** a new ADR and **not** a second collection mechanism: nothing yet shows the policy structurally unable to work. Registered in [`../reference/microvm-verification-harness.md`](../reference/microvm-verification-harness.md).

**The 4 GiB / 8 GiB pair remains argued.** Arm E ran at 1 GiB / 1.5 GiB, chosen so the gap would dominate `autoGC`'s 3% re-arm damper by an order of magnitude, which is a property of the _measurement image_ and says nothing about the shipped defaults.
