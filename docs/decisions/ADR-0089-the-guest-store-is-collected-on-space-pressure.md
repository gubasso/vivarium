...
Enacted 2026-08-04 by the `nix.settings` `min-free` and `max-free` lines in [`nix/guest.nix`](../../nix/guest.nix), replacing the interim posture recorded here before — that the guest never collected at all, because ADR-0088 makes a collection safe, which is not the same as arranging for one.
...
The trigger is measured, and its arithmetic is exact — 2026-08-04. `tests/host/store-pressure-check --arm c` drove the daemon's free-space reading down through upstream Nix's own `_NIX_TEST_FREE_SPACE_FILE` hook. The collector did not fire at 16, 8, 6, 5 or 4 GiB, fired at 3 GiB asking for 5,368,709,120 bytes, and fired at 2 GiB asking for 6,442,450,944. So it fires strictly below `min-free` — 4 GiB exactly is not a crossing — and the amount is exactly `max-free` minus available, which is this record's "collect until `max-free` is free again" confirmed on a running daemon. The premise underneath it is measured too: across every sample, `statvfs` on the merged `/nix/store` reported the same free blocks as the upper filesystem, so the trigger does read the store volume and nothing else. Registered in [`../reference/microvm-verification-harness.md`](../reference/microvm-verification-harness.md).

What is still not measured, and why the obvious experiment does not reach it. With free space faked, `availAfterGC` is faked too, so the above proves the trigger and the arithmetic and not real reclamation — whether a real pass on real ext4 actually returns the blocks the collector asked for. The attempt to get there by moving `min-free` close to a real volume's free space failed for a mechanical reason worth recording: `autoGC` reads `settings.minFree` inside the daemon, and neither a client-side `--option min-free` nor `NIX_USER_CONF_FILES` on the daemon unit reaches it. The only route proven to reach it is `nix.settings` at image-build time. Closing this needs a measurement image built with scaled thresholds; it is not reachable by a runtime override, and 3 GiB of real ballast on a 32 GiB volume never approaches the real 4 GiB threshold.

One question this record carried is withdrawn — 2026-08-04. It asked whether a `remountIfNecessary()` following a collection disturbs an in-flight build. Reading the pinned Nix, no such function exists: the only remount on the store path runs once from the `LocalStore` constructor, never after a collection. The open half above is one number, not a number plus a hazard. What that number must be measured with, and a damper on repeated passes, are recorded with the gap in [`../reference/microvm-verification-harness.md`](../reference/microvm-verification-harness.md).

The thresholds themselves are still argued. The numbers above validate the mechanism at the configured values, not the choice of 4 GiB and 8 GiB, which remains the sizing argument this record made from one Rust toolchain of headroom.

The trigger fires on a real filesystem and its arithmetic is exact — but the collection frees nothing, measured 2026-08-05. `tests/host/store-pressure-check --arm e`, on an image built with scaled thresholds and with upstream's free-space test hook removed, so every reading is real `statvfs` on real ext4. On the crossing iteration the daemon asked to free 754,675,712 bytes against 855,937,024 available and a 1.5 GiB target — exact to the byte, which confirms this record's "collect until `max-free` is free again" against a filesystem rather than against a hook.

What it did not do is reclaim. Guest free space did not recover and the dead-path count rose monotonically across the run and never fell. So this decision's mechanism is confirmed and its effect is not: as it stands, space-triggered collection does not bound the store volume in this topology. The cause is a stated hypothesis, not a measurement — the dead set is dominated by lower-layer paths a `local-overlay` store declines to delete, and `bytesFreed` is apparent size — and closing it needs the collector's per-path decisions instrumented, not another boot. Deliberately not a new ADR and not a second collection mechanism: nothing yet shows the policy structurally unable to work. Registered in [`../reference/microvm-verification-harness.md`](../reference/microvm-verification-harness.md).

The cause is measured, and it is not this policy — 2026-08-05. With the collector's own deletion decisions classified per path, on two independent boots, each collection attempted exactly one path, freed nothing, and reported its target met. The path was a host store path present through the lower share and unknown to both databases, and the pinned collector reads an uninitialised `bytesFreed` for exactly that case, declares the limit reached, and stops. So the hypothesis this record carried — thousands of lower-only paths satisfying a target cheaply — is refuted in its detail while its conclusion stands: the trigger and the arithmetic are exact, and the pass that follows does nothing. Tracked as [`../reference/known-issues/KI-0001/README.md`](../reference/known-issues/KI-0001/README.md); the mechanism is [`../reference/microvm-verification-harness.md`](../reference/microvm-verification-harness.md)'s.

This record is retained rather than revised. The defect is upstream's and is reached through [`ADR-0038-guest-store-sharing.md`](./ADR-0038-guest-store-sharing.md)'s lower layer, not through anything decided here, so nothing in the policy is wrong to change yet. What is now known rather than suspected is that the policy does not bound the volume while KI-0001 is open, and that a guard for it is a separate decision with its own evidence.

The 4 GiB / 8 GiB pair remains argued. Arm E ran at 1 GiB / 1.5 GiB, chosen so the gap would dominate `autoGC`'s 3% re-arm damper by an order of magnitude, which is a property of the measurement image and says nothing about the shipped defaults.
