...
- `viv config sources` renders both and exits `0`. It is the command you reach for after `start` returned `65`; a diagnostic that fails for the reason it exists to explain is useless. ADR-0030 sets the precedent — `status` reports `failed` at exit `0` because state is data.
