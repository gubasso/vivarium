...
instruction=$VIVARIUM_BENCH_HANDSHAKE_DIR/instruction
...
share_tree=$VIVARIUM_BENCH_HANDSHAKE_DIR/tree
...
rm -rf "$vol_tree"
cp -a "$share_tree" "$vol_tree" || {
  echo 'VIVARIUM_BENCH_SKIPPED=tree-copy-failed'
  exit 0
}

count_of() { find "$1" -type f | wc -l; }
digest_of() { find "$1" -type f -printf '%P %s\n' | sort | sha256sum | cut -d' ' -f1; }
sc=$(count_of "$share_tree")
vc=$(count_of "$vol_tree")
sd=$(digest_of "$share_tree")
vd=$(digest_of "$vol_tree")
...
for placement in share volume; do
  case $placement in
    share) cache_dir=$VIVARIUM_BENCH_HANDSHAKE_DIR/cache ;;
