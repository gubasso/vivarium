...
This directory is a self-contained npm sub-project. Nix owns the Node runtime through the repository devShell; npm owns Slidev itself, pinned by the committed `package-lock.json`. [`ADR-0104`](../docs/decisions/ADR-0104-the-published-deck-is-its-own-presentation-tree.md) records why the deck lives outside `docs/` and why the repository's Markdown regime does not apply here.
