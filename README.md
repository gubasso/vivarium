...
The tool is a thin command-line wrapper. It does the boring parts — resolving the manifest, building
the VM with Nix, mounting your working directory, wiring the network — and leaves the guest kernel,
the isolation boundary, and the merge semantics to well-established building blocks.

## Highlights

- **Strong isolation** — a real guest kernel behind a hardware-virtualization boundary, not a
  shared-kernel namespace.
- **Declarative & deterministic** — the sandbox is a Nix build; a pinned lockfile makes it
...
Design stage. The architecture is specified; the implementation has not started. See
[`docs/reference/implementation-status.md`](docs/reference/implementation-status.md) for what runs
today versus what is designed only.
